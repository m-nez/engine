...
int main(int argc, char** argv) {
	...
	render_states->dobjects[0].model_mat[14] -= 0.2;
	for(int i = 1; i < render_states->index[render_states->map->len]; ++i) {
		render_states->dobjects[i].model_mat[12] += 0.2 * i;
		render_states->dobjects[i].model_mat[13] += -0.03 * i;
	}
	GLuint text[8];

	render_states_realloc_bones(render_states);
